 ...
 * the root entity itself: {@code beach} ({@code Beach}), {@code municipality}
 * ({@code Municipality}), {@code region} ({@code Region}) and {@code favourite}
 * ({@code FavouriteBeaches}). Every value object in the domain — whether used
 * by a single aggregate or shared across several, e.g. {@code Name} — lives
 * together in {@code valueobject}. See each sub-package's
 * {@code package-info.java} for its specific boundary.
 *
